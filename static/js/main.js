...
async function main() {
  try {
    ...
    document.getElementById("start-process").addEventListener(
      "click",
      async () => controlProcess(processSelector.get(), "start"),
    );
    document.getElementById("stop-process").addEventListener(
      "click",
      async () => controlProcess(processSelector.get(), "stop"),
    );
    document.getElementById("block-process").addEventListener(
      "click",
      async () => controlProcess(processSelector.get(), "block"),
    );
    document.getElementById("unblock-process").addEventListener(
      "click",
      async () => controlProcess(processSelector.get(), "unblock"),
    );
    document.getElementById("restart-process").addEventListener(
      "click",
      async () => controlProcess(processSelector.get(), "restart"),
    );

    document.getElementById("toolbar-all-processes").addEventListener(
      "click",
      async () => {
    ...
    document.getElementById("toolbar-core-process").addEventListener(
      "click",
      async () => {
        // Change to the global scope by passing nothing
        changeLogScope("core");
