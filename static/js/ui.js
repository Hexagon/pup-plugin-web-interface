...
import {
  processConfigInventory,
  processSelector,
  processStatusInventory,
} from "./state.js";
import { getLogs } from "./network.js";

async function updateInstance(instanceDataIn, processDataIn) {
  const instanceData = instanceDataIn.data;
  const processData = processDataIn.data;
  const systemMemoryUsagePercent =
    ((instanceData.systemMemory.total - instanceData.systemMemory.free) /
      instanceData.systemMemory.total * 100).toFixed(2);
  setTextContentBySelector(
    "#system-memory-usage-percent",
    `${systemMemoryUsagePercent}%`,
  );
  if (systemMemoryUsagePercent > 90) {
    document.getElementById("system-memory-container").className =
      "bg-bad fg-bad";
  } else if (systemMemoryUsagePercent > 80) {
    document.getElementById("system-memory-container").className =
      "bg-warning fg-warning";
  }
  const systemMemoryUsage = instanceData.systemMemory.total -
    instanceData.systemMemory.free;
  setTextContentBySelector(
    "#system-memory-usage",
    `${formatBytes(systemMemoryUsage, false, "GB")} / ${
      formatBytes(instanceData.systemMemory.total, true, "GB")
    }`,
  );
  const systemSwapUsagePercent = ((instanceData.systemMemory.swapTotal -
    instanceData.systemMemory.swapFree) /
    instanceData.systemMemory.swapTotal * 100).toFixed(2);
  setTextContentBySelector(
    "#system-swap-usage-percent",
    `${systemSwapUsagePercent}%`,
  );
  const systemSwapUsage = instanceData.systemMemory.swapTotal -
    instanceData.systemMemory.swapFree;
  setTextContentBySelector(
    "#system-swap-usage",
    `${formatBytes(systemSwapUsage, false, "GB")} / ${
      formatBytes(instanceData.systemMemory.swapTotal, true, "GB")
    }`,
  );
  ...
  setTextContentBySelector(
    "#system-load",
    instanceData.loadAvg.map((l) => l.toFixed(2)).join(", "),
  );
  setTextContentBySelector(
    "#pup-memory-usage",
  ...
  setTextContentBySelector(
    "#pup-processes",
    instanceData.processes.length.toString(),
  );
...
export async function changeLogScope(processId) {
  ...
  if (processId && processId !== "core") {
    ...
    const selectedProcessCard = document.getElementById(
      `process-card-${sanitizedId}`,
    );
    ...
    document.getElementById("toolbar-current-process").classList.remove(
      "hidden",
    );
  ...
  logs.data.slice().reverse().forEach((log) => addLog(log, true));
}
...
function addLog(log, initial) {
  ...
  const logsDiv = document.getElementById("logs");
  let logsHtml = `<pre class="${log.processId}">${
    new Date(log.timeStamp).toLocaleString()
  } | ${log.category} | ${log.severity} > ${ansiToHtml(log.text)}\n</pre>`;
  if (initial) {
...
function updateSidebar(processId) {
  if (processId) {
    ...
    setTextContentBySelector(
      ...
      processConfig.cmd || processConfig.worker,
    );
    setTextContentBySelector(
      "#process-config-autostart",
      processConfig.autostart ? "Yes" : "No",
    );
    setTextContentBySelector(
      "#process-config-cron",
      processConfig.cron || "N/A",
    );
    setTextContentBySelector(
      "#process-config-restart",
      processConfig.restart || "always",
    );
    setTextContentBySelector(
      "#process-config-terminate",
      processConfig.terminate || "N/A",
    );
    setTextContentBySelector(
      "#process-config-cwd",
      ((processStatus.telemetry && processStatus.telemetry.cwd)
        ? processStatus.telemetry.cwd + " (telemetry)"
        : processConfig.cwd) || "N/A",
    );

    // Update process status values
    setTextContentBySelector(
      "#process-status-type",
      processStatus.type || "Unknown",
    );
    setTextContentBySelector(
      "#process-status-blocked",
      processStatus.blocked ? "Yes" : "No",
    );
    setTextContentBySelector(
      "#process-status-status",
      ProcessStateToString(processStatus.status),
    );
    setTextContentBySelector(
      "#process-status-restarts",
      processStatus.restarts || 0,
    );
    setTextContentBySelector(
      "#process-status-started",
      processStatus.started
        ? timeAgo(new Date(Date.parse(processStatus.started)))
        : "N/A",
    );
    setTextContentBySelector(
      "#process-status-exited",
      processStatus.exited
        ? timeAgo(new Date(Date.parse(processStatus.exited)))
        : "N/A",
    );
    setTextContentBySelector(
      "#process-status-updated",
      processStatus.updated
        ? timeAgo(new Date(Date.parse(processStatus.updated)))
        : "N/A",
    );
    setTextContentBySelector(
      "#process-status-telemetry",
      processStatus.telemetry ? "Connected" : "N/A",
    );
    setTextContentBySelector(
      "#process-status-memory",
      (processStatus.telemetry && processStatus.telemetry.memory)
        ? formatBytes(processStatus.telemetry.memory.rss || 0)
        : "N/A",
    );
    setTextContentBySelector(
      "#process-status-memory-heap-total",
      (processStatus.telemetry && processStatus.telemetry.memory)
        ? formatBytes(processStatus.telemetry.memory.heapUsed || 0) + " / " +
          formatBytes(processStatus.telemetry.memory.heapTotal || 0)
        : "N/A",
    );
    setTextContentBySelector(
      "#process-status-memory-external",
      (processStatus.telemetry && processStatus.telemetry.memory)
        ? formatBytes(processStatus.telemetry.memory.external || 0)
        : "N/A",
    );
